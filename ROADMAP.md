...
| # | 問題 | 深刻度 | 影響 | 修正方法 | 状態 |
|---|------|--------|------|----------|------|
| 1 | **Rolling stats データリーク** — `rolling().mean()` が当該試合を含む | CRITICAL | 精度1-3%水増し | `shift(1)` を追加して再訓練 | ⏳ |
| 2 | **確率キャリブレーション未実施** — XGBoostの`predict_proba()`をそのまま使用 | CRITICAL | 70%と言っても実際は64-66% | Platt scaling or isotonic regression で再訓練 | ⏳ |
| 3 | **Implied odds が捏造** — 実際のスポーツブックオッズを取得していない | HIGH | "edge"が無意味 | The Odds API等で実オッズ取得を検討 | ⏳ |
| 4 | **Confidence閾値が恣意的** — STRONG ≥ 8% edge の根拠がない | HIGH | 格付けに根拠なし | バックテストでティア別精度を検証し閾値を調整 | ⏳ |
| 5 | **F5モデルがフルゲーム特徴量** — 1-5回の結果予測にフルゲーム特徴量を使用 | MEDIUM | 予測精度が甘い | F5専用特徴量を設計して再訓練 | ⏳ |

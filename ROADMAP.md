...
```
WBC開幕: 2026年3月5日 → 残り6日
MLBシーズン開幕: 2026年3月27日 → 残り4週間
```
...
| 2/25 | データリーク修正（rolling stats shift(1)、overall_stats逐次累積） |
| 2/25 | ST予測データ生成・確率補正 |
| 2/27 | WBC記事06のファクトチェック修正（ベネズエラ所属国誤り・監督名） |
| 2/27 | Spring Training予測は非公開に決定（開幕3/27から本番） |
...
### バックテスト結果サマリー（2025年 2,425試合 — データリーク修正後）

> ⚠ 下記は2/25にrolling stats shift(1)・overall_stats逐次累積に修正した後の実測値。
> 旧値（ML 64.7%/72.9%等）はデータリークを含む水増し値だった。

| モデル | 全体精度 | STRONG精度 | STRONG試合数 |
|--------|---------|-----------|-------------|
| **Moneyline** | 52.9% | 58.7% | 550試合 |
| **Run Line** | 64.5% | 67.3% | 1,482試合 |
| **F5 Moneyline** | 54.5% | 59.9% | 1,096試合 |
| **NRFI** | 56.5% | 60.1% | 667試合 |
...
|---|--------|------|
| 1 | Substackに記事投稿 | ✅ |
| 2 | X (@MoneyballDojo) 自己紹介スレッド投稿 | ✅ |
| 3 | note.comに日本語版6本を投稿 | ⏳ |
| 4 | Beehiiv移行検討（購読者が増えてからでもOK） | ⏳ |
| 5 | Buffer アカウント作成 + X連携 | ⏳ |
...
## Phase 3: WBC + 開幕準備（3月上旬〜下旬）

### 方針決定（2/27）

```
Spring Training予測: 非公開（内部テストのみ）
  理由: ロスター未確定、投球制限、データ品質の問題
  → 公開すると信頼を失うリスクが高い

WBC予測: コンテンツ記事のみ（モデル予測なし）
  理由: WBCロスターはモデルの学習データ外
  → ファクト依存コンテンツはAPIハルシネーションの温床
  → 記事06でベネズエラ所属国誤り等を経験済み

本番開始: 3/27 Opening Day から
```

### WBC期間（3/5〜3/17）

```
WBC期間のコンテンツ（予測ではなく分析中心）:
  - WBC結果振り返り・ハイライト分析
  - WBCパフォーマンスのMLB開幕への影響分析
  - 「開幕Day全カード予測」予告コンテンツ（3/20頃）
```
...
| # | タスク | 状態 |
|---|--------|------|
| 1 | スプリングトレーニング予測データ生成 | ✅ 2/25完了（内部テスト用） |
| 2 | ST確率補正（shrinkage toward 0.5） | ✅ 2/25完了 |
| 3 | ST予測は非公開に決定（データ品質問題） | ✅ 2/27決定 |
| 4 | パフォーマンスダッシュボード作成 | ⏳ |

### 🚨 モデル品質改善（開幕前に必須）

2/25 のコード監査で発見。**開幕前に修正しないと信頼を失う。**

| # | 問題 | 深刻度 | 影響 | 修正方法 | 状態 |
|---|------|--------|------|----------|------|
| 1 | **Rolling stats データリーク** — `rolling().mean()` が当該試合を含む | CRITICAL | 精度1-3%水増し | `shift(1)` を追加して再訓練 | ✅ 2/25修正 |
| 2 | **overall_stats データリーク** — シーズン全体のstatsを使用 | CRITICAL | 精度水増し | 逐次累積計算に変更 | ✅ 2/25修正 |
| 3 | **確率キャリブレーション未実施** — XGBoostの`predict_proba()`をそのまま使用 | CRITICAL | 70%と言っても実際は64-66% | Platt scaling or isotonic regression で再訓練 | ⏳ |
| 4 | **Implied odds が捏造** — 実際のスポーツブックオッズを取得していない | HIGH | "edge"が無意味 | The Odds API等で実オッズ取得を検討 | ⏳ |
| 5 | **Confidence閾値が恣意的** — STRONG ≥ 8% edge の根拠がない | HIGH | 格付けに根拠なし | バックテストでティア別精度を検証し閾値を調整 | ⏳ |
| 6 | **F5モデルがフルゲーム特徴量** — 1-5回の結果予測にフルゲーム特徴量を使用 | MEDIUM | 予測精度が甘い | F5専用特徴量を設計して再訓練 | ⏳ |
| 7 | **WBC記事のハルシネーション** — Claude APIが未学習のロスター情報を捏造 | HIGH | 読者の信頼喪失 | ファクト依存コンテンツは検証済みソースを入力として渡す | ✅ 2/27修正 |

> **修正の優先順位:** #1 → #2 → #4 → #3 → #5
> #1と#2は `train_all_models.py` を修正して全モデル再訓練が必要。
> #3はスポーツブックAPI契約が必要（The Odds API: 無料枠あり）。

...
## 実績 (After) — 2026-02-26 最終結果

### Phase 1 (Tasks 1-4): 先発投手+Optuna+アンサンブル+Walk-forward
- **Moneyline: 57.8% → 60.0%** (+2.2%)
- **F5 Moneyline: 57.9% → 59.6%** (+1.7%)
- **Run Line: 64.6% → 66.3%** (+1.7%)

### Phase 2 (Tasks 5-8): 球場+休養日+移動距離+ドーム/標高
- **Moneyline: 60.0% → 60.3%** (+0.3%, Walk-forward avg: 60.0%)
- **F5 Moneyline: 59.6%** (維持)
- **Run Line: 66.3% → 65.9%** (微減、STRONG: 69.1%)
- **Over/Under: 54.9% → 57.9%** (+3.0% — park factorが効いた)

### 最終サマリー
- 特徴量: 28個 → 65個 (投手20 + 球場3 + 休養日3 + 移動距離3 + composite変動)
- アルゴリズム: XGBoost単体 → XGBoost+LightGBM+LogReg アンサンブル (ML)
- ハイパーパラメータ: Optuna 50 trials で最適化済み (ML, RL, F5)
- バリデーション: Walk-forward 2窓 (2022-23→2024: 60.3%, 2022-24→2025: 59.6%)
- 8. 実ラインナップ → 既存の打線力/投手特徴量でカバー済み（追加データなし）
